...
from apps.knowledge.extractors import DocxExtractionError
from apps.knowledge.verified import import_verified_docx, inspect_verified_docx


class Command(BaseCommand):
    help = "Inspect or import a human-reviewed DOCX file as trusted knowledge."

    def add_arguments(self, parser):
        parser.add_argument("path", help="A reviewed .docx Word file.")
        ...
        parser.add_argument(
            "--confirm-reviewed",
            action="store_true",
            help="Confirm that every fact and table was manually checked before trusted import.",
        )
        parser.add_argument(
            "--embed",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        try:
            if options["dry_run"]:
                inspection = inspect_verified_docx(
                    source,
            ...
            if not options["confirm_reviewed"]:
                raise CommandError(
                    "Trusted import requires --confirm-reviewed after every fact and table "
                    "has been checked."
                )

            result = import_verified_docx(
                source,
                source_key=options["source_key"],
                title=options["title"],
                embed=options["embed"],
            )
        except (DocxExtractionError, EmbeddingError, OSError) as exc:
