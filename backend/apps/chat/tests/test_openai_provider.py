...
class OpenAIProviderTests(SimpleTestCase):
    ...
    def test_stream_uses_private_bounded_responses_request(self, openai_client_class):
        ...
        self.assertIn("عبارت‌های منفیِ غیرضروری", request["instructions"])
        self.assertIn("مرز تأییدشده را روشن توضیح بده", request["instructions"])
        self.assertIn("گزینه مناسب‌تری", request["instructions"])
        self.assertIn("پنهان‌کردن محدودیت ایمنی یا فنی", request["instructions"])
        ...
        self.assertIn("در حافظه من نیست", request["instructions"])
        self.assertIn("گزینه‌های سریع: گزینه اول | گزینه دوم", request["instructions"])
