...
export function LogoMark({ compact = false }) {
  return (
    <div className="flex items-center gap-3" aria-label="دستیار هوشمند مشتریان">
      <span className={`${compact ? "h-11 w-11" : "h-14 w-14"} relative grid shrink-0 place-items-center rounded-2xl bg-teal-700 text-white shadow-lg shadow-teal-900/15`}>
        <Icon name="message" size={compact ? 24 : 30} />
        <span className="absolute -left-1 -top-1 h-3.5 w-3.5 rounded-full border-[3px] border-white bg-amber-400" />
      </span>
      <span className="text-right">
        <strong className={`${compact ? "text-base" : "text-lg"} block font-bold text-slate-900`}>دستیار هوشمند</strong>
        <span className="block text-xs font-medium text-slate-500">همراه مشتریان شما</span>
      </span>
    </div>
  );
}
